...
func TestAddTask(t *testing.T) {
	...
	tbl = []task{
		{"", "Заголовок", "", ""},
		{"20251220", "Сделать что-нибудь", "Хорошо отдохнуть", ""}, //Изменены даты, потому что изначально они не соответствуют условию. Должны быть больше текущей даты
		{"20251008", "Уроки", "", "d 10"},
		{"20250102", "Отдых в Сочи", "На лыжах", "y"},
		//{"today", "Фитнес", "", "d 1"}, Если я правильно понимаю, здесь ожидается результат : today + 1 день, но проверяется, что день текущий : "today && task.Date == now.Format(`20060102`)".
		//но тогда, где-то, в условиях, нужно внести в логику условие "task.Date = taskDate.AddDate(0, 0, 1).Format("20060102")", чтобы тест проходил корректно.
